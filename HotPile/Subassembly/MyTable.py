...
# 表头字段，全局变量
header_field = ['全选', '序号', '产品名称', '产品代号', '截面形状', '厚度', '应用方式', '弯曲情况', '高温工况\n传热能力','低温工况\n传热能力', '热流密度', '重量', '液塞', '孔数', '操作']
# 用来装行表头所有复选框 全局变量
all_header_combobox = []
...
class TableDemo(QWidget):
    ...
    def setTableWidget(self):
        ...
        self.tablewidget = QTableWidget(0,15)        # 3行4列
        self.tablewidget.verticalHeader().setVisible(False)
        self.tablewidget.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.Stretch)
        ...
        self.tablewidget.horizontalHeader().setSectionResizeMode(7, QHeaderView.Interactive)
        self.tablewidget.horizontalHeader().setSectionResizeMode(8, QHeaderView.Interactive)
        self.tablewidget.horizontalHeader().setSectionResizeMode(9, QHeaderView.Interactive)
        self.tablewidget.horizontalHeader().setSectionResizeMode(10, QHeaderView.Interactive)
        self.tablewidget.horizontalHeader().setSectionResizeMode(11, QHeaderView.Interactive)
        self.tablewidget.horizontalHeader().setSectionResizeMode(12, QHeaderView.Interactive)
        self.tablewidget.horizontalHeader().setSectionResizeMode(13, QHeaderView.Interactive)
    ...
    def setTableHeaderField(self):
        ...
        #设置标题栏高
        self.tablewidget.horizontalHeader().setMinimumHeight(50*gl.h)
        # self.tablewidget.horizontalHeader().setStretchLastSection(True)
        self.tablewidget.setColumnWidth(0,50*gl.w)       # 设置第0列宽度
        self.tablewidget.setColumnWidth(1,50*gl.w) 
        ...
        self.tablewidget.setColumnWidth(5,50*gl.w);
        ...
        self.tablewidget.setColumnWidth(12,50*gl.w);
    ...
    def setTableContents(self,oData,_shaixuanInfo):
        ...
        for row_number, row_data in enumerate(NewData):
            ...
            item=QtWidgets.QLabel()
            #高温传热能力
            ChuanRe_high=ProductSelect.GetChuanReNengli(float(dicshaixuanInfo['hightemp']),row_data['prtkong20'],1)
            item.setText(str(ChuanRe_high))
            if ChuanRe_high-ProductSelect.CalTotalgonglv(self.shaixuanInfo,1)<1e-6:
                item.setStyleSheet("color: rgb(255, 0, 0);")
            self.tablewidget.setCellWidget(row_number,8,item)
            item=QtWidgets.QLabel()
            #低温传热能力
            ChuanRe_low=ProductSelect.GetChuanReNengli(float(dicshaixuanInfo['lowtemp']),row_data['prtkong20'],1)
            item.setText(str(ChuanRe_low))
            if ChuanRe_low-ProductSelect.CalTotalgonglv(self.shaixuanInfo,2)<1e-6:
                item.setStyleSheet("color: rgb(255, 0, 0);")
            self.tablewidget.setCellWidget(row_number,9,item)
            self.tablewidget.setItem(row_number, 10, QtWidgets.QTableWidgetItem(str(row_data["midulinjie20"])))
            item=QtWidgets.QLabel()
            ...
            if "突显" in str1:
                ...
                item.setText(str1)
            self.tablewidget.setCellWidget(row_number,12,item)
            self.tablewidget.setItem(row_number, 13, QtWidgets.QTableWidgetItem(str(row_data["kongnum"])))
            self.tablewidget.setCellWidget(row_number, 14, self.buttonForRow(row_data["id"]))
